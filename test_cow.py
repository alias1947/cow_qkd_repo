...
def run_two_node_cow_simulation(link_distance_km=20, num_pulses_per_link=10000, mu=0.2,
    ...
    alice_key, bob_key = temp_network.nodes[node_ids[0]].generate_and_share_key_cow(
...
def test_two_node_cow_qber_vs_distance(num_trials=15):
    ...
    for d in distances:
        ...
        for _ in range(num_trials):
            key_len, qber = run_two_node_cow_simulation(link_distance_km=d)
            qbers.append(qber)
    ...
    plt.title(f'COW Average QBER vs Link Distance (2 nodes, {num_trials} trials)')
    plt.grid(True)
    plt.show()

if __name__ == "__main__":
    test_two_node_cow_qber_vs_distance(num_trials=10) 